...
if (!Jmol._version)
    Jmol = (function (document) {
        ...
        var j = {
            _version: "$Date: 2017-04-27 20:28:06 -0500 (Thu, 27 Apr 2017) $", // svn.keywords:lastUpdated
            _alertNoBinary: true,
...
......<div id=\"ID_appletdiv\" style=\"z-index:" + Jmol._getZ(applet, "header") + ";width:100%;height:98%;position:absolute;top:0px;left:0px;" + css + ">";
...
(function (tm) {
    ...
    tm._CPK = ["#FF1493", "#FFFFFF", "#D9FFFF",
        // 'Li',     'Be',      'B',        'C',      'N',       'O',       'F',      'Ne',
        "#CC80FF", "#c9d5e9", "#FFB5B5", "#797979", "#3050F8", "#FF0D0D", "#90e001", "#B3E3F5",
        // 'Na',      'Mg',     'Al',     'Si',       'P',      'S',       'Cl',       'Ar',
        "#AB5CF2", "#bbc7db", "#BFA6A6", "#F0C8A0", "#FF8000", "#eeee2c", "#419941", "#80D1E3",
        // 'K',       'Ca',     'Sc',      'Ti',       'V',       'Cr',     'Mn',        'Fe',
        "#8F40D4", "#bbc7db", "#E6E6E6", "#BFC2C7", "#A6A6AB", "#8A99C7", "#9C7AC7", "#E06633",
        // 'Co',  	'Ni', 		'Cu', 		'Zn', 		'Ga', 	'Ge',		 'As', 		'Se', 	'Br', 		'Kr',
        "#F090A0", "#50D050", "#C88033", "#7D80B0", "#C28F8F", "#668F8F", "#BD80E3", "#FFA100", "#A62929", "#5CB8D1",
        // 'Rb',     'Sr',       'Y',      'Zr',      'Nb',      'Mo',      'Tc',     'Ru',
        "#702EB0", "#bbc7db", "#94FFFF", "#94E0E0", "#73C2C9", "#54B5B5", "#3B9E9E", "#248F8F",
        // 'Rh',     'Pd',      'Ag',       'Cd',      'In',     'Sn',       'Sb',      'Te',      'I',      'Xe',
        "#0A7D8C", "#006985", "#C0C0C0", "#FFD98F", "#A67573", "#668080", "#9E63B5", "#D47A00", "#940094", "#429EB0",
        // 'Cs',     'Ba',
        "#57178F", "#bbc7db"];  // through barium
...
// A user-available function
        ...
        proto._loadFile = function (fileName) {
            ...
            var app = this;
            Jmol._loadFileData(this, fileName, function (data) {
                app.__loadModel(data)
            });
        };
...
//  adaptable ---
        ...
        proto._parseSDF = function (sdf) {
            ...
            for (var i = 0; i < this.nAtoms; i++) {
                ...
                this.elements[ept++] = this.atoms[i] = {
                    type: 0,
                    elemNo: elemNo,
                    xyz: [x, y, z],
                    sxyz: [0, 0, 0],
            ...
            for (var i = 0; i < this.nBonds; i++) {
                line = lines[pt++];
                var a1 = this.atoms[parseFloat(line.substring(0, 4)) - 1];
                var a2 = this.atoms[parseFloat(line.substring(4, 8)) - 1];
                var order = Math.abs(parseFloat(line.substring(8, 11)));
                ...
                this.elements[ept++] = this.bonds[i] = {
                    type: 1,
                    atoms: [a1, a2],
                    xyz: xyz,
                    pts: [p1, p2],
