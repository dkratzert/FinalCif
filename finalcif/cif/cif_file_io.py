...
class CifContainer():
    ...
    def atoms(self, without_h: bool = False) -> Generator:
        ...
        for label, type, x, y, z, part, occ, u_eq in zip(labels, types, x, y, z,
            ...
            if without_h and self.ishydrogen(label):
                continue
            if self.picometer:
                u_eq = angstrom_to_x(u_eq, squared=True)
            #         0    1   2  3  4   5   6     7
    ...
    def displacement_parameters(self):
        ...
        for label, u11, u22, u33, u23, u13, u12 in zip(labels, u11, u22, u33, u23, u13, u12):
            if self.picometer:
                u11 = angstrom_to_x(u11, squared=True)
                u22 = angstrom_to_x(u22, squared=True)
                u33 = angstrom_to_x(u33, squared=True)
                u23 = angstrom_to_x(u23, squared=True)
                u13 = angstrom_to_x(u13, squared=True)
                u12 = angstrom_to_x(u12, squared=True)
    ...
    def hydrogen_bonds(self) -> Generator:
        ...
        for label_d, label_h, label_a, dist_dh, dist_ha, dist_da, angle_dha, symm, publ in (
            ...
            if self.yes_not_set(publ):
                continue
            if self.picometer:
                dist_dh = angstrom_to_x(dist_dh)
                dist_ha = angstrom_to_x(dist_ha)
                dist_da = angstrom_to_x(dist_da)
