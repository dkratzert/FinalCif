...
## Changes by Daniel Kratzert to make it more compatible for general use.
...
unicode_math_table_file = str((Path(__file__).parent / 'unicode-math-table.tex').resolve())
unicode_math_xetex_file = str((Path(__file__).parent / 'unicode-math-xetex.sty').resolve())
...
if unicode_math_xetex_file is not None:
    for n, l in enumerate(open(unicode_math_xetex_file)):
        for a, b in S.findall(l) + R.findall(l):
            ...
            if codes is None:
                if verbose >= 3:
                    syslogger('(file unicode-math-xetex.sty line %d unknown %s in %r)\n' % (n, b, l))
                continue
