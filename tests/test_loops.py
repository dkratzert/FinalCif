...
class TestLoops(AppWindowTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.app.ui.LoopsPushButton.click()
    ...
    def get_index_of(self, loopkey: str = ''):
        tabw: QTabWidget = self.app.ui.LoopsTabWidget
        tab = -1
    ...
    def test_delete_loop(self):
        self.assertEqual(9, len(self.app.cif.loops))
        # Index 0 is the author editor
        self.assertEqual(1, self.app.ui.LoopsTabWidget.currentIndex())
        self.app.ui.deleteLoopButton.click()
        self.assertEqual(8, len(self.app.cif.loops))

    def test_get_index_of_tab(self):
        self.assertEqual(2, self.get_index_of('CIF Author'))

    def test_loop_data(self):
        index = self.app.ui.LoopsTabWidget.widget(self.get_index_of('Citations')).model().index(0, 1)
        self.assertEqual('10.1021/acs.orglett.0c01078', index.data())

    def test_loop_data_atoms_aniso(self):
        index = self.app.ui.LoopsTabWidget.widget(self.get_index_of('Displacement Para')).model().index(0, 1)
        self.assertEqual('0.0161(10)', index.data())

    def test_loop_audit_author_name(self):
        index = self.app.ui.LoopsTabWidget.widget(self.get_index_of('CIF Author')).model().index(0, 0)
        self.assertEqual('Daniel Kratzert', index.data())

    def test_loop_audit_author_address(self):
        index = self.app.ui.LoopsTabWidget.widget(self.get_index_of('CIF Author')).model().index(0, 1)
        self.assertEqual(unify_line_endings('University of Freiburg\nGermany'), unify_line_endings(index.data()))

    def test_loop_dots(self):
        index = self.app.ui.LoopsTabWidget.widget(self.get_index_of('Bonds')).model().index(1, 3)
        self.assertEqual('.', index.data())

    def test_loop_question_marks(self):
        index = self.app.ui.LoopsTabWidget.widget(self.get_index_of('Bonds')).model().index(1, 4)
        self.assertEqual('?', index.data())

    def test_loop_no_edit(self):
        self.app.ui.SaveCifButton.click()
        c = CifContainer(self.app.cif.finalcif_file)
        val = c.loops[3][0, 2]
        self.assertEqual('0.0181', val)

    def test_loop_edit_one_single_field(self):
        model = self.app.ui.LoopsTabWidget.widget(self.get_index_of('Scattering')).model()
...
class TestLoopsMove(AppWindowTestCase):
    ...
    def set_current_index_to_row_col(self, row, col, tab='Scattering'):
        view = self.app.ui.LoopsTabWidget.widget(self.get_index_of(tab))
    ...
    def test_order_of_scattering_factors_table(self):
        ...
        """
        view = self.app.ui.LoopsTabWidget.widget(self.get_index_of('Scattering'))
