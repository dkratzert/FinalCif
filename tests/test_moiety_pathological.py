...
def unit_cell_content(cif: CifContainer, z: int) -> dict[str, float]:
    ...
    symmetry-expanded; they contribute their occupancy once per formula unit,
    matching how the generator treats them.
    """
    ...
    for atom in special:
        content[_normalize_element(atom[1])] += float(atom[6]) * z
    return dict(content)
...
class TestPathologicalModels:
    ...
    def test_mass_is_conserved(self, stem, description, platon_reference):
        ...
        """
        cif, result = _result(stem)
        expected = unit_cell_content(cif, result.z)
